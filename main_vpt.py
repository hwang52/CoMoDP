...
if __name__=="__main__":
    ...
    model = build_vpt_promptmodel(num_classes=args.task_classes, img_size=args.image_size, 
                                  model_idx='ViT', patch_size=args.patch_size, 
                                  Prompt_Token_num=args.vpt_num, VPT_type=args.vpt_type) # or VPT_type="Shallow"
    ...
    if args.evaluate:
        print("=> evaluate vpt prompt tuning")
        vpt_prompt_path = os.path.join('./save', 'fgvc_cub_vpt_Deep_result.pt')
        print("=> loading checkpoint '{}'".format(vpt_prompt_path))
        checkpoint_dict = torch.load(vpt_prompt_path)
        model.load_prompt(checkpoint_dict)
        model.to(args.device)
        evaluate_acc = test_model(model, test_loader, args.device)
        print('---------------- evaluate vpt tuning acc {vpt_acc}'.format(vpt_acc=evaluate_acc))
    else:
        print("=> training vpt prompt tuning")

        best_acc = 0
        step = 0
        best_epoch = 0
        test_acc_list = []
        for epoch in tqdm(range(args.epochs), desc='VPT-training'):
            ...
            if best_acc < test_acc:
                ...
                json.dump(state, open(os.path.join('./save/', f'{args.dataset}_vpt_{args.vpt_type}-result.json'), 'w'), indent=4)
                # save model
                vpt_state_dict = model.obtain_prompt()
                torch.save(vpt_state_dict, os.path.join('./save/', f'{args.dataset}_vpt_{args.vpt_type}_result.pt'))
            else:
                # save result
                state = {'best_epoch':best_epoch, 'best_acc':best_acc, 'test_acc_list':test_acc_list}
                json.dump(state, open(os.path.join('./save/', f'{args.dataset}_vpt_{args.vpt_type}-result.json'), 'w'), indent=4)
        ...
        # vpt_acc = test_model(model, test_loader, args.device)
        # print('---------------- after vpt acc {acc}'.format(acc=vpt_acc))
        print("=> VPT test accuracy: ", test_acc_list, " | VPT max accuracy: ", max(test_acc_list))
        # python3 main_vpt.py --dataset fgvc_cub --task_classes 200 --device 'cuda:0'