...
class NABirds(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=None):
        ...
        dict_keys = list(self.label_map.keys())
        all_names = list(self.class_names.values())
        label_names = []
        for i in range(len(dict_keys)):
            label_names.append(all_names[dict_keys[i]])
...
if __name__ == '__main__':
    train_dataset = NABirds('../dataset/nabirds', train=True)
    test_dataset = NABirds('../dataset/nabirds', train=False)
    print(len(train_dataset))
    print(len(test_dataset))
    print(train_dataset[-1]) # Swallow-tailed Kite
    dict_keys = list(train_dataset.label_map.keys())
    print(dict_keys)
    all_names = list(train_dataset.class_names.values())
    print(all_names[295])
    print(train_dataset.label_names[0])