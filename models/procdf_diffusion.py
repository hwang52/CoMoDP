...
class SDFeaturizer:
    def __init__(self, sd_id='stabilityai/stable-diffusion-2-1', null_prompt='', args=None):
        ...
        gc.collect()
        self.args = args
        onestep_pipe = onestep_pipe.to(args.device)
        # onestep_pipe = onestep_pipe.to('cuda:0')
    ...
    def forward_ensemble(self,
        ...
        # img_tensor = img_tensor # b,c,h,w
        img_tensor_list = []
        for single_img in img_tensor:
            single_img = single_img.repeat(ensemble_size, 1, 1, 1) # ensem,c,h,w
            img_tensor_list.append(single_img)
        prompt_list = prompt
        prompt_emb_list = []
        for prompt in prompt_list:
            ...
            prompt_emb_list.append(prompt_embeds)
        # prompt_embeds = prompt_embeds.repeat(ensemble_size, 1, 1)
        # prompt_embeds = prompt_embeds # b,77,dim
        unet_ft_list = []
        for img_tensor,prompt_embeds in zip(img_tensor_list,prompt_emb_list):
            unet_ft_all = self.pipe(
                img_tensor=img_tensor, # ensem,c,h,w
                t=t,
                up_ft_indices=[up_ft_index],
                prompt_embeds=prompt_embeds) # ensem,77,dim
            unet_ft = unet_ft_all['up_ft'][up_ft_index] # ensem, c, h, w
            unet_ft = unet_ft.mean(0, keepdim=True) # 1,c,h,w
            unet_ft_list.append(unet_ft)
        # unet_ft = unet_ft_all['up_ft'][up_ft_index] # b,c,h,w
        # unet_ft = unet_ft.mean(0, keepdim=True) # 1,c,h,w 
        unet_ft_list = torch.cat(unet_ft_list, dim=0) # b,c,h,w 
        return unet_ft_list 
    ...
    def forward(self,
        ...
        if prompt == self.null_prompt:
            ...
            prompt_embeds = self.pipe._encode_prompt(
                ...
                # device='cuda:0',
                num_images_per_prompt=1,
                do_classifier_free_guidance=False) # [b, 77, dim]
    ...
    def forward_list(self,
        ...
        unet_ft_all = self.pipe(
            ...
            prompt_embeds=prompt_embeds)
        # unet_ft = unet_ft_all['up_ft'] # b, c, h, w
        return unet_ft_all 
...
if __name__=='__main__':
    ...
    layer_id = 4
    dift_fea = dift_extractor.forward(img_tensor=img_tensor, prompt=prompt, t=0, up_ft_index=layer_id-1)
    # up_ft_index=0: [b, 1280, 7, 7]
    # up_ft_index=1: [b, 1280, 14, 14]
    # up_ft_index=2: [b, 640, 28, 28]
    # up_ft_index=3: [b, 320, 28, 28]
    print(dift_fea.shape)
    dift_fea = dift_fea.cpu().numpy()
    dift_fea = np.mean(dift_fea, axis=1).squeeze()
    normalized_feature_maps = (dift_fea - np.min(dift_fea)) / (np.max(dift_fea) - np.min(dift_fea))
    # plt.imshow(normalized_feature_maps, cmap='viridis')
    plt.imshow(normalized_feature_maps, cmap='hot', interpolation='nearest')
    plt.colorbar()
    plt.show()
    plt.savefig(f'../flower_hot_layer{layer_id}.jpg', bbox_inches="tight", dpi=600, pad_inches=0.0)
    # cv2.imwrite(f'../f_heat_layer{layer_id}.jpg', heat_map, [int(cv2.IMWRITE_JPEG_QUALITY),100])